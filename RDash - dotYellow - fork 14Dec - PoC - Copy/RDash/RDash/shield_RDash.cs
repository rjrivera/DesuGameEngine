...
namespace RDash
{
    class shield_RDash : VFX
    {
        ...
        */
        ...
        public shield_RDash(Texture2D loadedTexture)//i shouldn't use this constructor...
        {
            rotation = 0.0f;
        ...
        public shield_RDash(float initX, float initY, float initRotation, float initXCenter, float initYCenter,
            float initXVelocity, float initYVelocity, float backgroundX, float backgroundY, int sprites)
        {
            ...
            spriteWidth = 45; //hardcoded to enforce class uniqueness in design
            spriteHeight = 45;
        ...
        public override void  Update(float gameTime, float backgroundX, float backgroundY, Vector2 dukePosition)
        {
            ...
            if (spriteTimer > spriteInterval)//possiblity of updateing the interval itself for advanced behavior control.
            {
                ...
                {
                    currentFrame = 0;
                    ////ttl--; no ttl because it is a powerup dependant on an asynchronous trigger, not a clock.  
                }
                sourceRect = new Rectangle(currentFrame * spriteWidth, 0, spriteWidth, this.mainSpriteSheet.Height);
            }
            this.position.X = dukePosition.X + backgroundX + 10f;
            this.position.Y = dukePosition.Y + backgroundY + 10f;
